...
df <- df %>% 
  ...
  filter(!Target %in% c("il6", "tnfα")) %>% # remove il6 and tnfα from the analysis as the expression levels are too low 
...
df$Target <- factor(df$Target, 
                    levels = c("myd88", "il1β", "il8", "cd3γδ", "cd8β", "mhcI", 
                               "ifnγ","il17a", "foxp3", "il10", "tgfβ1", "il4", 
                               "plin2", "mtp", "apoa1", "apoa4", "apob", "chk", 
                               "pcyt1a", "fabp2b", "aqp8ab", "cldn15", "cldn25b", 
                               "zo1", "cdh1", "muc2",  "cyp1a1", "mta", "hsp70", 
                               "sod1", "cat", "casp6", "pcna", "mmp13")) 
...
df %>% 
  ...
  labs(title = "Gene expression profile in PI", y = "MNE") +
  cowplot::theme_cowplot() +
  scale_fill_brewer(palette = "Dark2")

ggsave('analysis/exploratory_analysis/qPCR_target_PI_violin.pdf', 
       units = "in", 
       dpi = 300, 
...
ggsave('analysis/exploratory_analysis/qPCR_target_PI_boxPlot.pdf', 
       units = "in", 
       dpi = 300, 
       encoding = "CP1253", 
       height = 24, 
...
# Make heatmap
pdf('analysis/exploratory_analysis/qPCR_target_PI_heatmap.pdf', 
    encoding = "CP1253", 
    height = 10,
    width = 8.5)
...
# 13 fitted models are singular. We'll not fit LMMs to these variables, instead 
...
ggexport(filename = "analysis/model_diagnostics/qPCR_PI_welch-t_qqplot.pdf",
...
pdf(file = "analysis/model_diagnostics/qPCR_PI_LMMs_residual.pdf", 
    encoding = "CP1253",
    width = 8) 
...
# Based on the residual analysis, we'll run influence analysis for apoa4, apob, 
# mtp and pcna. 
...
for (var in c("apoa4", "apob", "mtp", "pcna")) { 
...
# Make heatmap
tiff('results/figures/Figure 3.tiff', 
     compression = "lzw", 
     units = "in", 
     res = 300,
     height = 10,
...
ggsave('results/figures/Figure S1.tiff', 
       units = "in", 
       dpi = 300, 
       compression = "lzw",
       height = 10, 
...
# Get session info 
writeLines(capture.output(sessionInfo()), "analysis/code/qPCR_target_PI_sessionInfo.txt")
